...
public class SearchControllerTest {
	...
	void 즐겨찾기한_영화_찾기() throws Exception {
		ResultActions getMovieListByUserId = mvc.perform(get("/favorite-movies/user/fhdufhdu").param("page", "0"));
	...
	@Test
	@WithMockCustomUser(username = "admin", roles = Role.ADMIN)
	void 즐겨찾기한_영화_찾기_관리자() throws Exception {
		ResultActions getMovieListByUserId = mvc.perform(get("/favorite-movies/user/fhdufhdu").param("page", "0"));
		getMovieListByUserId
				.andExpect(jsonPath("$.content.length()", is(2)));
	}
	...
	@Test
	@WithMockCustomUser(username = "fhdufhdu", roles = Role.USER)
	void 즐겨찾기_등록하기() throws Exception {
		FavoriteMovieAddDto favoriteMovieAddDto = FavoriteMovieAddDto.builder()
				.userId("fhdufhdu")
				.movieId(3L)
				.build();
		ResultActions addFavorite = mvc.perform(post("/favorite-movies")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(favoriteMovieAddDto)));
		addFavorite
		...
		favoriteMovieAddDto.setMovieId(2L);
		ResultActions addFavorite1 = mvc.perform(post("/favorite-movies")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(favoriteMovieAddDto)));
		addFavorite1
				.andExpect(status().isBadRequest());

		favoriteMovieAddDto.setUserId("admin");
		ResultActions addFavorite2 = mvc.perform(post("/favorite-movies")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(favoriteMovieAddDto)));
		addFavorite2
				.andExpect(status().isBadRequest());
	...
	@Test
	@WithMockCustomUser(username = "admin", roles = Role.ADMIN)
	void 즐겨찾기_등록하기_관리자() throws Exception {
		FavoriteMovieAddDto favoriteMovieAddDto = FavoriteMovieAddDto.builder()
				.userId("fhdufhdu")
				.movieId(3L)
				.build();
	...
	@Test
	@WithMockCustomUser(username = "fhdufhdu", roles = Role.USER)
	void 즐겨찾기_제거() throws Exception {
		ResultActions removeFavorite = mvc.perform(delete("/favorite-movies/1"));
		removeFavorite
				.andExpect(status().isOk());
		...
		ResultActions removeFavorite1 = mvc.perform(delete("/favorite-movies/3"));
		removeFavorite1
				.andExpect(status().isBadRequest());
	}
	...
	@Test
	@WithMockCustomUser(username = "admin", roles = Role.ADMIN)
	void 즐겨찾기_제거_관리자() throws Exception {
		ResultActions removeFavorite = mvc.perform(delete("/favorite-movies/1"));
		removeFavorite
				.andExpect(status().isOk());
