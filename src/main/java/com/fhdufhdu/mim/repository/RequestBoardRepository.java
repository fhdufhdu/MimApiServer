...
public interface RequestBoardRepository extends JpaRepository<RequestBoard, Long> {
    @Query(value = "select r from RequestBoard r join r.movie m where r.isConfirm = false", nativeQuery = false)
    Page<RequestBoard> findAll(Pageable pageable);

    @Query(value = "select r from RequestBoard r where r.id = :id and r.isConfirm = false", nativeQuery = false)
